...
8. **Database Transformation Sync** — Once stored in `external.academic_lgu_events`, the PostgreSQL trigger `tg_sync_academic_lgu_events` evaluates the post via `external.classify_event_from_text` (equipped with resilient, tense-agnostic regex, constrained statutory holiday matches, and `#WalangPasok` hashtag matching) to automatically insert qualified disruption events into `external.events_consolidated`:
   ...
   - **Traffic & Number Coding Advisory Routing**: MMDA/LGU Number Coding, road traffic rerouting, and motorist advisories (e.g. *"Abiso sa mga motorista"*, *"mabagal na daloy ng trapiko"*, *"paligid ng EDSA People Power Monument"*) are strictly classified as `lgu` under `CIVIC_MAINTENANCE` and prohibited from masquerading as school holidays or class suspensions.
   - **Student Welfare Lounges & Exam Cheer Suppression**: Student council welfare lounges, free coffee/snack distribution booths (`study fuel`, `snack booth`, `free coffee`, `student activity room`, `busking`), and motivational social media exam cheer greetings (`give it your best`, `you got this`, `fighting`) are categorized under non-disruptive administrative or regular class items (`affects_ridership = FALSE`), preventing internal student welfare activities from generating spurious multi-day exam spikes or duplicating institutional academic calendars (`purge_scraped_events_sept25_anomalies_and_tighten_classifier.sql`).
   - **Hotel Ballroom & Municipal Awards Isolation**: Municipal awards ceremonies, banquets, and recognition programs held inside enclosed private hotel ballrooms (e.g. Novotel Manila Araneta City ballroom for the *31st Quezon City Barangay Day Celebration*) are explicitly excluded from `MAJOR_ARENA_EVENT` via `novotel`, `ballroom`, `hotel function`, and `function hall` exclusion patterns (`affects_ridership = FALSE`), preventing hotel ballroom ceremonies from falsely triggering 0.50–0.65 transit arena shocks.
