...
6. **Off-Corridor Mega-Venue Geofence Pre-Filter (`is_off_corridor_venue()`)** — Blocks events taking place at major sports/entertainment arenas outside the LRT-2 corridor transit catchment walkshed (e.g. SM Mall of Asia Arena Pasay, PICC Plenary Hall Pasay, San Andres Sports Complex Malate, Rizal Memorial Stadium Pasay/Manila, Ninoy Aquino Stadium, Philsports Arena/Ultra Pasig, Cuneta Astrodome, Filoil EcoOil Centre San Juan). Prevents non-corridor mega-events from generating spurious transit capacity spikes across LRT-2 stations.
7. **Micro-Venue & Administrative Deadlines Filter (`is_micro_venue_or_administrative()`)** — Filters out ticket sales booths, dance studios, covered court rehearsals, and administrative form deadlines (e.g. admissions, transcript clearance, graduation form submissions) from being classified as `MAJOR_ARENA_EVENT` (weight 0.65).
