...
| Technology | Purpose |
| --- | --- |
| Python | Main ingestion and orchestration language |
...
|-- ingest_raw_internal.py
|-- ingest_student_transaction.py
|-- ingest_station_platform_capacity.py
|-- ingest_psor_incidents.py
...
| Script | Dataset | Target Schema/Table |
| --- | --- | --- |
| `ingest_raw_internal.py` | AFCS ridership files by month/year | `AFCS.ridership_<year>` |
| `ingest_student_transaction.py` | Student transaction counts | `AFCS.student_transactions` |
| `ingest_station_platform_capacity.py` | Station and platform capacity data | `Station Capacity.station_platform_capacity` |
| `ingest_psor_incidents.py` | PSOR incident records | `PSOR.psor_incidents` |
| `run_all_ingestions.py` | Runs all internal ingestion scripts in sequence | Orchestration only |
...
The scripts are defensive: they validate required columns, expected file names, dates, numeric values, and required text fields before writing to the database.
...
- SQL identifiers are built with `psycopg2.sql`.
