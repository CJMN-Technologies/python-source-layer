...
| `keywords.py` | Pre-filter: classifies post text as `academic`, `lgu`, or irrelevant using keyword groups aligned to the friction weight table. Uses `.casefold()` for case-insensitive matching. Equipped with negative keyword exclusion guards for post-flood medical/health notices (Leptospirosis, Doxycycline) and relief goods distribution to prevent false weather disruption triggers. |
